...
public class SongInformation implements Validatable {
	...
	public SongInformation(BMSModel model) {
        int[] totalNotesCombined = BMSModelUtils.getTotalNotesCombined(model);
		sha256 = model.getSHA256();
		n = totalNotesCombined[BMSModelUtils.TOTALNOTES_KEY];
		ln = totalNotesCombined[BMSModelUtils.TOTALNOTES_LONG_KEY];
		s = totalNotesCombined[BMSModelUtils.TOTALNOTES_SCRATCH];
		ls = totalNotesCombined[BMSModelUtils.TOTALNOTES_LONG_SCRATCH];
		total = model.getTotal();
		
		int[][] lanenotes = new int[model.getMode().key][3];
		int[][] data = new int[model.getLastTime() / 1000 + 2][7];
		int pos = 0;
		int border = (int) (totalNotesCombined[BMSModelUtils.TOTALNOTES_ALL] * (1.0 - 100.0 / model.getTotal()));
		int borderpos = 0;
		...
		final int bd = totalNotesCombined[BMSModelUtils.TOTALNOTES_ALL] / data.length / 4;
		density = 0;
		peakdensity = 0;
		int count = 0;

...
public class SongManagerMenu {
    ...
    public static void show(ImBoolean showSongManager) {
        ...
        if (ImGui.begin("Song Manager", showSongManager, ImGuiWindowFlags.AlwaysAutoResize)) {
            ...
            if (currentCourseData.isPresent()) {
                songName = currentCourseData.get().getName();
            }
    ...
    private static Optional<ScoreData> getCurrentScoreData() {
        if (selector.getSelectedBar() instanceof SongBar songBar) {
            return Optional.ofNullable(songBar.getScore());
        } else if (selector.getSelectedBar() instanceof GradeBar gradeBar) {
	        return Streams.of(gradeBar.getScore(), gradeBar.getMirrorScore(), gradeBar.getRandomScore())
	                .filter(Objects::nonNull)
	                .max(Comparator.comparing(ScoreData::getDate));
        }
        return Optional.empty();
    }

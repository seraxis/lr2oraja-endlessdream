...
import bms.player.beatoraja.modmenu.skin.debugger.SkinOptions;
	...
	public LR2SkinCSVLoader(Resolution src, Config c) {
			...
			public void execute(String[] str) {
				...
				if (divx * divy >= 10) {
					...
					if (images != null) {
						...
						if (property == null) {
							SkinOptions.registerMissingNumberDefinition(values[11]);
						}
		...
		addCommandWord(new CommandWord("SRC_TEXT") {
			...
			public void execute(String[] str) {
				...
				if (property == null) {
					SkinOptions.registerMissingTextDefinition(values[3]);
				}
